...
//  match
//------------------------------------------
  if (isset($key_array1[1])) {
    if ($key_array1[1]=='mch') {
      $del_res01=$obj->del_syain_tachi01(ACCESSID,'del');
      if ($del_res01 == TRUE) {
        $strsql ="";
        ...
        if ($ins_res02[0] == 1) {
          ...
          //echo 'gggggg';
          $this->_redirect('upd_check12',ACCESSID .','.'xxxxx'.','.'su9');
          exit;
        }
      } else {
...
//  form 要素の構築
//------------------------------------------
$q_kei_c= $form->addElement('text','q_kei_c',array('style' => 'width: 30px;border:none;font-size:14px;color:#ff0099;text-align:right;background-color: #eeeeee;','readonly'=>'readonly'),array('label' =>"q_kei:"));
$m_kei_c= $form->addElement('text','m_kei_c',array('style' => 'width: 30px;border:none;font-size:14px;color:#3399ff;text-align:right;background-color: #eeeeee;','readonly'=>'readonly'),array('label' =>"MAS:"));
$t_kei_c= $form->addElement('text','t_kei_c',array('style' => 'width: 30px;border:none;font-size:14px;color:#3399ff;text-align:right;background-color: #eeeeee;','readonly'=>'readonly'),array('label' =>"TRN:"));

$q_kei_c -> addrule('regex', 'q_kei: '.ERRCNT0,'/^[0-9]+$/');
$m_kei_c -> addrule('regex', 'q_kei: '.ERRCNT0,'/^[0-9]+$/');
$t_kei_c -> addrule('regex', 'q_kei: '.ERRCNT0,'/^[0-9]+$/');
//$q_kei_c->addrule('required',KEYEMPTY);
...
if (isset($_FILES['upfile']['error']) && is_int($_FILES['upfile']['error'])) {
    ...
    //------------------------------------
        ...
        if (isset($executed)) {
           ...
           // $msg = array('green', 'Upload successful');
            ...
            //-------------------------------------------------
            $stmt = $pdo->prepare('SELECT access_id,syainno,grade as license,bsy_nm as filler1,birth_md,company_id  FROM  t_syain_sun_wk01');
            $stmt->execute();
            while ( $rows = $stmt->fetch ( PDO::FETCH_ASSOC ) ) {
              ...
              $ins_res02=$obj->upd_syain_tachi01(
            ...
            //--------------------------------------------------
               ...
               for ( $i = 0; $i < $cnt_i; ++$i){
                    ...
                    if (preg_match('/^[ｦ-ﾟｰ ]+$/u', $ary_syain01[$i][6])) {
                        $q6_1 = $q6_1 + 1;
                      $q6_1_who=$q6_1_who . ','. $ary_syain01[$i][1];
                      } else {
                    }
                    if (strlen($ary_syain01[$i][6]) <= 48) {
                    } else {
                      $w6_1 = $w6_1 + 1;
                    ...
                    //  } else {
                    //  $q6_2 = $q6_2 + 1;
                    //  $q6_2_who=$q6_2_who . ','. $ary_syain01[$i][1];
                    // }
                     //-----------------
                    // Company id
                    //-----------------
            ...
            $res0 =$obj->upd_syain_chk01(CLIENTID,'q0',$q0,$q0_who,'rew');
            $res1 =$obj->upd_syain_chk01(CLIENTID,'q1',$q1,$q1_who,'rew');
            $res2 =$obj->upd_syain_chk01(CLIENTID,'q2',$q2,$q2_who,'rew');
            $res5 =$obj->upd_syain_chk01(CLIENTID,'q5',$q5,$q5_who,'rew');
            $res6 =$obj->upd_syain_chk01(CLIENTID,'q6',$q6,$q6_who,'rew');
            $res11 =$obj->upd_syain_chk01(CLIENTID,'q11',$q11,$q11_who,'rew');
            //
            $res0_1 =$obj->upd_syain_chk01(CLIENTID,'q0_1',$q0_1,ltrim($q0_1_who,","),'rew');
            $res1_1 =$obj->upd_syain_chk01(CLIENTID,'q1_1',$q1_1,ltrim($q1_1_who,","),'rew');
            $res1_2 =$obj->upd_syain_chk01(CLIENTID,'q1_2',$q1_2,ltrim($q1_2_who,","),'rew');
            $res1_2w =$obj->upd_syain_chk01(CLIENTID,'w1_2',$w1_2,ltrim($w1_2_who,","),'rew');
            $res2_1 =$obj->upd_syain_chk01(CLIENTID,'q2_1',$q2_1,ltrim($q2_1_who,","),'rew');
            $res2_2 =$obj->upd_syain_chk01(CLIENTID,'q2_2',$q2_2,ltrim($q2_2_who,","),'rew');
            $res2_1w =$obj->upd_syain_chk01(CLIENTID,'w2_1',$w2_1,ltrim($w2_1_who,","),'rew');
            $res5_1 =$obj->upd_syain_chk01(CLIENTID,'q5_1',$q5_1,ltrim($q5_1_who,","),'rew');
            $res5_2 =$obj->upd_syain_chk01(CLIENTID,'q5_2',$q5_2,ltrim($q5_2_who,","),'rew');
            $res6_1 =$obj->upd_syain_chk01(CLIENTID,'q6_1',$q6_1,ltrim($q6_1_who,","),'rew');
            $res6_1w =$obj->upd_syain_chk01(CLIENTID,'w6_1',$w6_1,ltrim($w6_1_who,","),'rew');
            $res11_1 =$obj->upd_syain_chk01(CLIENTID,'q11_1',$q11_1,ltrim($q11_1_who,","),'rew');
            $q_kei=0;
            $q_kei=$q0+$q1+$q2+$q5+$q6+$q11+$q0_1+$q1_1+$q1_2+$q2_1+$q2_2+$q5_1+$q6_1+$q5_2+$q11_1;
            //echo $q_kei;
            $res2 = array();
            $ary_chk01 = array();
            $res2 = $obj ->getlist01_01('select * from t_syain_chk01 where client_id =? order by client_id,q_id',CLIENTID);
               $cnt_i=count($res1[RESDATA]);
               $cnt_j=count($res1[RESDATA][0]);
               //echo 'cnt_i-->' . $cnt_i .'    cnt_j-->' . $cnt_j;
  ...
  $res3 = array();
  $ary_sum01 = array();
  $res3 = $obj ->getlist01('select sum(cnts) from t_syain_chk01');
  $ary_sum01 =$this -> sanitize($res3[RESDATA]);
  //
  $res4 = array();
  $ary_sum04 = array();
  $strsql4 ='';
  $strsql4 ='select count(*) from t_syain_sun where access_id=? ';
  $res4 = $obj ->getlist01_01($strsql4,ACCESSID);
 ...
 //-------------------------------------------
  ...
  if (isset($_FILES['upfile']['name'])) {
   ...
   $q_kei_c->setvalue($ary_sum01[0][0]);
