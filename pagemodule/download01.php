...
function main()
{
  ...
  //------------------------------------------
	...
	$kaiko_fr_c = $form->addElement('select','kaiko_fr_c',array('class' => 'states','style' => 'width:120px;heigth:25px;font-size:14px;'),array('options' =>$ary_kaiko))->setlabel("Kaiko_fr:");
	$kaiko_to_c = $form->addElement('select','kaiko_to_c',array('class' => 'states','style' => 'width:120px;heigth:25px;font-size:14px;'),array('options' =>$ary_kaiko))->setlabel("Kaiko_to:");
    ...
    $kaiko_chk_c = $form->addElement('text','kaiko_chk_c',array('style' => 'width: 10px;display: none;'),array('label' => "kaiko_chk_c:"));
    $rec_seq_c = $form->addElement('text','rec_seq_c',array('class' => 'form-control','style'=>'width:110px;height:20px;'),array('label' => "Rec_SEQ"));
    //-------------------------------------------------
	//  初期値
	//--------------------------------------------------
	...
	$rec_seq_c->addrule('required','rec_seq');
	//-------------------------------------------
	//  Filter
	//-------------------------------------------
    ...
    $rec_seq_c ->addfilter('htmlspecialchars');
    //-------------------------------------------
	//  kaiko fr-to check
	//-------------------------------------------
	...
	if (isset($_POST['sendmail01'])) {
		$this->_redirect('sendmail01');
		//exit();
	} else {
	}
	if (isset($_POST['apa'])) {
		//$this->_redirect('apache_log01');
		//exit();
	} else {
	}
	if (isset($_POST['wget01'])) {
		$this->_redirect('wget01');
		//exit();
	} else {
	}
	//-------------------------------------------
	...
	if ($form->validate())  {
		if (isset($_POST['send0']) ) {
          ...
          $data = str_replace('2018/05/28',$_POST['nko_ymd_c'], $data);
          //$ans = file_put_contents("template/Data_guide_temp02.html" , $data);
          $ans = file_put_contents("template/Seiseki_guide.html" , $data);
