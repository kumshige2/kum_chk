...
//  instance
//------------------------------------------
$form = new HTML_Quickform2('qform_smarty','POST',array('action' => 'index.php?module=juk_knr_syain_view01'));
//-------------------------------------------
//  Post Get Session
//-------------------------------------------
  ...
  if ($key_array1[3]=='upl') {
	...
	ChromePhp::log('juk_knr_syain_view01');
//---------------------------------------------
//  Post key check
//  $_POST['KEY']  通常表示
//  $_POST['id']   更新、追加、削除
...
if(isset($sts)) {
  switch ($sts){
    ...
    //-------------------------------
      ...
      $res0 = $obj -> getlist02_01($sqls,$key_array1[0],$key_array1[4]);
    ...
    // t_syain_sun
    //-------------------------------
      $sqls="select a.access_id,a.syainno,a.birth_md,a.bsy_cd,a.bsy_nm,a.license as grade ,a.filler1,a.filler2,a.filler3,a.filler4,a.except_flg,a.company_id,a.client_id,to_char(a.upd_ymd,'yyyy-mm-dd'),to_char(cre_ymd,'yyyy-mm-dd') from t_syain_sun as a where a.access_id=? order by a.syainno";
    ...
    // t_syain_sun
    //-------------------------------
     $sqls="select a.access_id,a.syainno,a.birth_md,a.bsy_cd,a.bsy_nm,a.license as grade ,a.filler1,a.filler2,a.filler3,a.filler4,a.except_flg,a.company_id,a.client_id,to_char(upd_ymd,'yyyy-mm-dd'),to_char(cre_ymd,'yyyy-mm-dd')  from t_syain_sun as a where a.access_id=? order by a.syainno";
      $res1 = $obj  -> getlist01_01($sqls,$_SESSION[ACCESSID]);
      break;
  }
  if (count($res1[RESDATA])  != 0) {
    ...
    //$this -> smarty->assign('sf',$sf1);
       ...
       echo "<span sytle=\"color:#FF0000\">" . 't_syain_sun empty !?'  . "</span>";
  $cnt = 0;
  }
} else {
  echo 'NULL';
...
$q_id_c= $form->addElement('text','q_id_c',array('style' => 'display:none'),array('label' =>"q_id:"));
$cnt_c= $form->addElement('text','cnt_c',array('style' => 'display:none'),array('label' =>"cnt:"));
//
//$access_id_c= $form->addElement('text','access_id_c',array('style' => 'width:50px;'),array('label' =>"access_id:"));
//$client_id_c= $form->addElement('text','client_id_c',array('style' => 'width:50px;'),array('label' =>"client_id:"));
//$client_nm_c= $form->addElement('text','client_nm_c',array('style' => 'width:50px;'),array('label' =>"client_nm:"));
//$sts_c= $form->addElement('text','sts_c',array('style' => 'width:50px;'),array('label' =>"sts_c:"));
//$q_id_c= $form->addElement('text','q_id_c',array('style' => 'width:50px;'),array('label' =>"q_id:"));
//------------------------------------
//初期値を取得
//------------------------------------
if ($cnt === 0) {
...
if (isset ($_POST['excel01'])) {
  ...
  //$ares = $obj -> upd_pmh_log01('PHP:Juk_knr_sys:syain download',$cnt,$sqls,get_current_user(),
...
$this->smarty->assign('ary_syn_view', $this->pub_syn_view00);
...
//$val_array2=  $this ->json_safe_encode($rows);
  ...
  $this->smarty->assign("data_json", $val_array2);
	...
	$this->smarty->display('juk_knr_syain_view01.html');
	}// FUNCTION
