...
class layout_detail02_ins   extends view {
 //$property
  ...
  public $pub_key_res01;
  public $pub_key_uns01;
	public $pub_fld00;
  public $pub_fld01;
  public $pub_fld02;
  public $pub_fld03;
...
//------------------------------------------
  ...
  if (isset( $_POST[P_ADDDELETE])) {
    if ($_POST['judge'] ==1) {
     //delete
        $del_res01 =$obj->del_tbl01($key_array1[1].','. $key_array1[2].','. 0);
        if ($del_res01 == TRUE) {
           //echo "bbbbb";
           $this->_redirect('upd_check01', $key_array1[1].','.$key_array1[2].','.'sd0');
        } else {
         //echo "cccc";
         $this->_redirect('upd_check01', $key_array1[1].','.$key_array1[2].','.'sd9');
         exit;
        }
...
//--------------------------------------------------
  ...
  $fld_ret = $this -> setChannel($ary_fld,'fld_view');
  ...
  //---------------------------------------------------
    ...
    //------------------
      if ($sts=='rew') {
       ...
       $sqls = $sqls . ' WHERE s.tbl_id=?  AND s.tbl_seq=? and s.tbl_kbn=? ';
       $sqls = $sqls . ' ORDER BY upd_ymd desc,s.tbl_id asc,s.tbl_seq asc,s.tbl_kbn asc ';
       $res1 = $obj -> getlist03_01($sqls,$key_array1[1],intval($key_array1[2]),500);
       //選択済FIELD
       //echo 'res1:sqls--->'.$sqls;
       $sqls ='';
       $sqls = ' select  s.* ';
       ...
       $sqls = $sqls . ' ORDER BY upd_ymd desc,s.tbl_id asc,s.tbl_seq asc,s.tbl_kbn asc ';
       //$res1 = $obj -> getlist03_01($sqls,$key_array1[1],0,0);
       $res11 = $obj -> getlist03_01($sqls,$key_array1[1],intval($key_array1[2]),intval($key_array1[3]));
       //$ret01 = $obj ->table($res1[RESDATA][0]);
  ...
  //--------------------
      ...
      //$x3 = $obj ->table($this->pub_fld03);
       //
  		...
  		$cnt_res1 =0;
  		echo('ary_crs_view =0 -->' . $key_array1[1] . ':' .$key_array1[2].':' .$key_array1[3]);
      exit;
  	}
...
//  form 要素の構築
//------------------------------------------
	$memos_c= $form->addElement('text','memos_c',array('style' => 'background-color:#eeeeee;width: 100%;border-bottom: solid 3px #66ff99;border-left: none;border-top:none;border-right:none;font-size:18px;'),array('label' =>"memos:"));
  ...
  //--------------------------------------------------
    case 'ins':
      if (isset($_POST['upd'])) {
        ...
        //---------------------------------------
        $ret_upd =$this->upd_tbl_sub01($sts,$key_array1[1],$strr,$key_array1[3],$memos_w);
     ...
     // -----------
      if (isset($_POST['upd'])) {
        ...
        if ($del_res01 == TRUE) {
         ...
         $this->_redirect('upd_check01', $key_array1[1].','.$key_array1[3].','.'sd9');
         exit;
        }
        $del_res01 =$obj->del_tbl01($key_array1[1].','. $key_array1[2].','. 500);
        if ($del_res01 == TRUE) {
        } else {
          $this->_redirect('upd_check01', $key_array1[1].','.intaval($key_array1[3])+500 .','.'sd9');
          exit;
        }
        //--------------------------------------------
        ...
        //--------------------------------------------
        //echo 'aaaa';
        $ret_upd =$this->upd_tbl_sub01($sts,$key_array1[1],$key_array1[2],$key_array1[3],$memos_w);
        if ($ret_upd == true )  {
          $this->_redirect('upd_check01',$tbl_id_w .','.$p_no_latest.','.'su0');
        } else {
          $this->_redirect('upd_check01',$tbl_id_w .','.$p_no_latest.','.'su9');
...
//}
	...
	function setChannel($row01,$sw){
      ...
      break;
			...
			break;
			case 'key_res1':
      $this->pub_key_res01 =$row01;
      break;
      case 'key_uns1':
      $this->pub_key_uns01 =$row01;
      break;
      case 'sel_view':
...
//  update t_tbl_sub01
//----------------------------------
 function upd_tbl_sub01($sts, $key1, $key2,$key3,$memos_w){
//--------------------------------------------
//  insert array create  $data_fld
//--------------------------------------------
//echo 'aaaa';
...
//  public $pub_key_uns01;
  $obj = new addressoperation;
  $data_fld=array();
  $data_fld=array($this->pub_fld_view00[RESDATA][0]=>$key1);
  $data_fld+=array($this->pub_fld_view00[RESDATA][1]=>$key2);
  $data_fld+=array($this->pub_fld_view00[RESDATA][2]=>$key3);
  //
  for($i = 3 ; $i < count($this->pub_fld_view00[RESDATA])-3; $i++) {
    if (isset($this->pub_key_res01[$i-3])) {
      $fld00 = $this->pub_key_res01[$i-3];
    } else {
      $fld00 = NULL;
    }
    $data_fld+=array($this->pub_fld_view00[RESDATA][$i] => $fld00);
  }
//
  ...
  $ret02 = $obj ->ins_fld($query,'t_tbl_sub01',$this->pub_fld_view00[RESDATA],$data_fld);
  //--------------------------------------------
  //  insert array create  $data_fld field
  //  未使用FIELDの登録 fld_kbn =500
  //--------------------------------------------
  //echo 'cnt--->' .count($ary_crs_view);
  $key3_500=0;
  $key3_500 = intval($key3) + 500;
  //
  $data_fld=array();
  $data_fld=array($this->pub_fld_view00[RESDATA][0]=>$key1);
  $data_fld+=array($this->pub_fld_view00[RESDATA][1]=>$key2);
  $data_fld+=array($this->pub_fld_view00[RESDATA][2]=>$key3_500);
  //
  for($i = 3 ; $i < count($this->pub_fld_view00[RESDATA])-3; $i++) {
    if ($i > intval(count($this->pub_crs_view00))) {
      $data_fld+=array($this->pub_fld_view00[RESDATA][$i] => NULL);
    } else {
      $data_fld+=array($this->pub_fld_view00[RESDATA][$i] => $this->pub_key_uns01[$i-3]);
    }
  } //end for
  $data_fld+=array('memo' => $memos_w);
  $data_fld+=array('upd_ymd' => date("Y-m-d H:i:s"));
  ...
  $query  = "INSERT INTO t_tbl_sub01 ";
  $query .= "VALUES " . implode("," ,array_fill(0, count($data_fld), "(?, ?)"));
  $ret021 = $obj ->ins_fld($query,'t_tbl_sub01',$this->pub_fld_view00[RESDATA],$data_fld);
  //echo ('ret21--->'.$ret021);
  //rew
  if (($ret02 == true ) || ($ret021 == true)) {
    return true;
    //this->_redirect('upd_check01',$tbl_id_w .','.$key3.','.'su0');
  } else {
    return false;
    //$this->_redirect('upd_check01',$tbl_id_w .','.$key3.','.'su9');
    //echo "<span sytle=\"color:#FF0000\">" . 't_pno_com --->' . ADDUPDATENG . "</span>";
  }
  $obj->dbend();
 } // end function
