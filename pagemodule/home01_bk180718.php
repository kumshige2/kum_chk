 ...
 //var $smarty;
  ...
  //---------------------------------------------	  	  
	...
	//--------------------------------------------------	
	 ...
	 $res1 = array();
     $fr_ym =  substr($this_ndo_2,2,2) . '04';
	 $to_ym =  substr($next_ndo,2,2). '03';
	 $res1 = $obj -> getlist03_01($sqls,$fr_ym,$to_ym,$key_bsy);
	 if ($res1[RESFLAG] == TRUE) {
		$res11 = $this -> sanitize($res1[RESDATA]);
		echo 'count--->' . count($res1[RESDATA]);
	 } else {
		echo 'data exception --->' .$sql2;		 
	 }
	 
	 $res3= $obj -> tables($res1);
	 $res0 = array();
	 for ($i = 0; $i < 36; $i++){ 
	   if ($res11[$i][0] == $key_ym) {
	     $res0[0] = $res11[$i];
		 //echo '---2002--->' . $res0[0][5];
		 //for($j=0;$j < 30;$j++) {
		 //  $ary_xx= array_push($ary_xx,$res11[$i][$j]);
		// }
	   }
	 }
	 $res3= $obj -> tables($res0);
	 exit;
	 //
	 //$sqls ="";
	 //$sqls ='SELECT * FROM t_jisseki_wk01 as a '; 
	 //$sqls = $sqls. ' where a.keijo_ym = ? and a.bsy_cd = ? ';
	 //$sqls = $sqls. ' order by a.keijo_ym,a.bu_cd,a.bsy_cd ';
	 //
     //$res0 = $obj -> getlist02_01($sqls,$key_ym,$key_bsy);
	 //$res1= $obj -> tables($res0);
	 //exit;
