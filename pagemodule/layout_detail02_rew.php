...
//print_r($_SESSION);
//echo "</pre>";
class layout_detail02_rew   extends view {
 //$property
	var $smarty;
	public $pub_rows00;
...
//--------------------------------------------------
  ...
  //exit;
//--------------------------------------------------
// 標準パターンより選択済Fieldを取得(when sts=rew)
//t_tbl_sub01  omit fld get
//--------------------------------------------------
  if ($sts=='rew') {
  	$res_omt = array();
    $ary_omt_fld = array();
    ...
    $sqls = $sqls . 'WHERE s.tbl_id=?  AND s.tbl_seq=? and s.tbl_kbn=500 ';
    $sqls = $sqls . 'ORDER BY upd_ymd desc,s.tbl_id asc,s.tbl_seq asc,s.tbl_kbn asc ';
    //
    $res_omt = $obj -> getlist02_01($sqls,$key_array1[1],$key_array1[2]);
    if (count($res_omt[RESDATA][0])> 0) {
      $cnt_res_omt = count($res_omt[RESDATA][0]);
      for ($i=3; $i<$cnt_res_omt; $i=$i+1) {
         if (isset($res_omt[RESDATA][0][$i])) {
            $ary_omt_fld00[] = $res_omt[RESDATA][0][$i];
         } else {
           //echo 'Null field--->' . $i . '--->' . $res1[RESDATA][0][$i-1];
           break;
         }
      }
      $ary_omt_fld = $this -> sanitize($ary_omt_fld00);
     //
      $rr = sort($ary_omt_fld);
     // $ret201 = $obj ->table($ary_omt_fld);
      //-------------------------------------------
      //  select field create
      //-------------------------------------------
      $fld99="";
      $sj =0;
      //echo 'cnt-omt-->' . count($ary_omt_fld);
      for ($i=0; $i < count($ary_fld[RESDATA]);$i++) {
        if (count($ary_omt_fld)==0) {
          $fld99 = $fld99 . $ary_fld[RESDATA][$i] . ',';
        } else {
          for ($j=0; $j < count($ary_omt_fld);$j++) {
          switch (true) {
            case ($i === intval($ary_omt_fld[$j])):
             //echo '='. $i;
             array_shift($ary_omt_fld);
             break 2;
            case ($i < intval($ary_omt_fld[$j])):
             $fld99 = $fld99 . $ary_fld[RESDATA][$i] . ',';
             $sj=0;
      ...
      echo('ary_fld_view =0 -->' . $key_array1[1] . ':' .$key_array1[2]);
      exit;
    }
  } else
  {
     echo('sts ?? ' . $sts . '-->'. $key_array1[1] . ':' .$key_array1[2]);
      exit;
  }
//--------------------------------------------------
//t_tbl_sub01  res1
//--------------------------------------------------
  //---------------------------------------------------
  // Sts=Ins（新規）の場合 tbl_seq=0,tbl_kbn=0 固定値
  // Sts=Ins（新規）≠場合 tbl_seq=$_Post,tbl_kbn=$_Post
  //---------------------------------------------------
  $res1 = array();
  $res11 = array();
  $res12 = array();
  ...
  //選択済FIELD  $res11
   ...
   //選択済FIELD  $res12
    ...
    $sqls = $sqls . ' ORDER BY upd_ymd desc,s.tbl_id asc,s.tbl_seq asc,s.tbl_kbn asc ';
   //$res1 = $obj -> getlist03_01($sqls,$key_array1[1],0,0);
    $res12 = $obj -> getlist03_01($sqls,$key_array1[1],$key_array1[2],50);
    //$ret12 = $obj ->table($res12[RESDATA][0]);
   //echo 'cnt--->' . count($res1[RESDATA][0]);
  } else {
    echo('sts ?? ' . $sts . '-->'. $key_array1[1] . ':' .$key_array1[2]);
    exit;
...
//---------------
$ary_num_view = array();
$ary_num_view00 = array();
$cnt_res12 =0;
  if (is_array($res12[RESDATA])) {
    $cnt_res12 = count($res12[RESDATA][0]);
    for ($i=3; $i<$cnt_res12; $i=$i+1) {
      if (isset($res12[RESDATA][0][$i])) {
        //echo 'num--->'.$res12[RESDATA][0][$i];
        $fldxx = $this ->fld_cnv02(intval($res12[RESDATA][0][$i]));
        //$fld00 = $ary_crs_view[$fldxx];
        $ary_num_view00[] = $fldxx;
      } else {
       //echo 'Null field--->' . $i . '--->' . $res1[RESDATA][0][$i-1];
       break;
      }
    }
    $ret12 = $obj ->table($ary_num_view00);
    $ary_num_view = $this -> sanitize($ary_num_view00);
    $ret12 = $this -> setChannel($ary_num_view,'num_view');
  } else {
     echo "<pre>";
     echo('ary_num_view =0 -->' . $key_array1[1] . ':' .$key_array1[2].':' . 50 );
     echo "</pre>";
    exit;
  }
//----------------------------------------------------
...
//  form 要素の構築
//------------------------------------------
	//$memos_c= $form->addElement('text','memos_c',array('style' => 'width: 93%;border-bottom: solid 3px #00e7ff;border-left: none;border-top:none;border-right:none;font-size:18px;'),array('label' =>"memos:"));
  //class="jquery-ui-sortable"
  $memos_c= $form->addElement('text','memos_c',array('style' => 'width: 100%;height:75px;font-size:14px;','title'=>'備考'),array('label' =>"memos:"));
  ...
  //--------------------------------------------------
    case 'rew':
      if (isset($_POST['upd'])) {
        ...
        $query='';
        $query  = "update  t_tbl_sub01 ";
        $query .= "set " . implode("," ,array_fill(0, count($data_fld), "(?, ?)"));
        echo 'sql---->' > $query;
        exit;
        ...
        $data_fld=array($ary_fld[RESDATA][0]=>$key_array1[1]);
        $data_fld+=array($ary_fld[RESDATA][1]=>$strr);
        $data_fld+=array($ary_fld[RESDATA][2]=>$key_array1[3]+50);
        //
        for($i = 3 ; $i < count($ary_fld[RESDATA])-3; $i++) {
         if (isset($key_res1[$i-3])) {
           $fldxx = $this ->fld_cnv01($key_res1[$i-3]);
         ...
         $data_fld+=array($ary_fld[RESDATA][$i] => $fldxx);
        }
        $data_fld+=array('memo' => $memos_w);
        $data_fld+=array('upd_ymd' => date("Y-m-d H:i:s"));
        $data_fld+=array('cre_ymd' => date("Y-m-d H:i:s"));
...
function createForm($form,$sts){
  ...
  $this->smarty->assign('ary_num_view', $this->pub_num_view00);
  ...
  $this->smarty->assign('ary_json01', $this->pub_json01);
	...
	if ($sts=='ins') {
    $this->smarty->display('layout_detail01_ins.html');
  } else {
