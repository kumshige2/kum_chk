...
function dispChannel(){
  ...
  if (empty($_POST['p_no_c'])) {
   ...
   $res1 = $obj  -> getlist01($sqls);
     //ChromePhp::log('------- indsert -------');	
   //ChromePhp::log('count:' . $count);	  
...
FROM
	    ...
	    if ($res1[RESFLAG] == TRUE ) {	   
           ...
           for ( $i = 0; $i <= 20; ++$i){
			 $ary_settei[] = e($res1[RESDATA][0][$i]);        
			  ...
			  echo '<p>' .  't_settei_inf  invalid (他団体パンフ !?):'  . "</p>\n";
			  $ary_settei = array();
 ...
 //-----------------------------------------
    ...
    //-----------------------------------------
	 ...
	 $objSheet->setCellValue("B27",e($_POST['card_hyojun_memo_c']));
...
//-------------------------------------------	
  switch ($_POST['pmh_type_c']) {
    case 'sw':
        //$objSheet->getHeaderFooter()->setOddHeader('&K0000ff' . mb_convert_encoding('&C--- ＷＥＢ --- ', 'UTF-8'));
	$objSheet->getHeaderFooter()->setOddHeader('&K0000ff' . mb_convert_encoding('&L Pamphlet発注票　　---　WEB ---                                                                                '. date("Y/m/d"), 'UTF-8'));
		 break;
	 case 'tw':
    $objSheet->getHeaderFooter()->setOddHeader('&K0000ff' . mb_convert_encoding('&L Pamphlet発注票　　---　WEB ---                                                                                '. date("Y/m/d"), 'UTF-8'));
		 break;
...
//---------------------------------    
  ...
  switch (count($ary_settei)) {
   ...
   if (count($ary_settei) ==0) {
	  $objSheet->setCellValue('B43',$val .  NULL);   
   } else {
  ...
  switch (count($ary_settei)) {
    ...
    default:
     ...
     if (count($ary_settei) ==0)  {
	   $objSheet->setCellValue('B44',$val .  NULL);   
   } else {
  ...
  switch  (count($ary_settei)) {
   ...
   //$objSheet->setCellValue('G40',$val);  
    if (count($ary_settei) ==0)  {
	   $objSheet->setCellValue('G40',  NULL);   
   } else {
	    $objSheet->setCellValue('G40',$val); 
   }
 //-----------------------------------------
 ...
 //-----------------------------------------
  ...
  //'keisai_crs_add_c'
 if (count($ary_settei) ==0)  {
	$objSheet->setCellValue('G41',NULL); 
    $objSheet->setCellValue('G42',NULL); 
 } else {
 ...
 //-----------------------------------------         
     //otr_t_c
 if (count($ary_settei) ==0)  {
	 $objSheet->setCellValue('B45',NULL);
 } else {	 
	 $val= e($ary_settei[03]);
 ...
 //-----------------------------------------     
 if (count($ary_settei) ==0)  {
	 $objSheet->setCellValue('A46',NULL);  
 } else {
	  ...
	  //rpt_rnh_kbn
 if (count($ary_settei) ==0)  { 
      $objSheet->setCellValue('E46',NULL); 
 } else {
  ...
  case '2':
    ...
    //  
    //-----------------------------------------
      $objSheet->getHeaderFooter()->setOddHeader('&K0000ff' . mb_convert_encoding('&L Pamphlet確認票　　 　　　                         ＊このシートは作成状況を確認するシートです!  '. date("Y/m/d"), 'UTF-8'));
	 ...
	 $val0 = intval(str_replace(',','',$_POST['pmh_cost_c']));
	 $objSheet->setCellValue('B34', $val0);
	 $objSheet->getStyle('B34')->getNumberFormat()->setFormatCode('"\"#,##0');
	 
	 $objSheet->setCellValue('D34',$_POST['senyu_page_c']);
	 //$objSheet->setCellValue('F34',$_POST['page_tanka_c']);
	 $val0 = intval(str_replace(',','',$_POST['page_tanka_c']));
	 $objSheet->setCellValue('F34', $val0);
	 $objSheet->getStyle('F34')->getNumberFormat()->setFormatCode('"\"#,##0');
	
	 //$objSheet->setCellValue('B35',$_POST['post_cost_c']);
	 $val0 = intval(str_replace(',','',$_POST['post_cost_c']));
	 $objSheet->setCellValue('B35', $val0);
	 $objSheet->getStyle('B35')->getNumberFormat()->setFormatCode('"\"#,##0');
	 ...
	 $val0 = intval(str_replace(',','',$_POST['bes_cost_c']));
	 $objSheet->setCellValue('B36', $val0);
	 $objSheet->getStyle('B36')->getNumberFormat()->setFormatCode('"\"#,##0');
	 

	 $objSheet->setCellValue('D36',$_POST['com_page_c']);
		 
	 //$objSheet->setCellValue('B37',$_POST['any_cost_c']);
	 $val0 = intval(str_replace(',','',$_POST['any_cost_c']));
	 $objSheet->setCellValue('B37', $val0);
	 $objSheet->getStyle('B37')->getNumberFormat()->setFormatCode('"\"#,##0');
	 ...
	 $valc = $vala -  $valb;  
	 //echo '<p>val:-->' . $valc . '</p>';   
	 $objSheet->setCellValue('D39',$valc);
     $objSheet->getStyle('D39')->getNumberFormat()->setFormatCode('"\"#,##0');
	 
	 //$objSheet->setCellValue('D39',$_POST['tdan_sek_c']);
	 
	 //$objSheet->setCellValue('D40',$_POST['sanno_keisai_cost_c']);
	 $val0 = intval(str_replace(',','',$_POST['sanno_keisai_cost_c']));
	 $objSheet->setCellValue('D40', $val0);
	 $objSheet->getStyle('D40')->getNumberFormat()->setFormatCode('"\"#,##0');
	 //$objSheet->setCellValue('F40',$_POST['com_cost_c']);
	 $val0 = intval(str_replace(',','',$_POST['com_cost_c']));
	 $objSheet->setCellValue('F40', $val0);
	 $objSheet->getStyle('F40')->getNumberFormat()->setFormatCode('"\"#,##0');
	 //$objSheet->setCellValue('H40',$_POST['mng_cost_c']);
	 $val0 = intval(str_replace(',','',$_POST['mng_cost_c']));
	 $objSheet->setCellValue('H40', $val0);
	 $objSheet->getStyle('H40')->getNumberFormat()->setFormatCode('"\"#,##0');
	 $objSheet->setCellValue('D37',$_POST['sho_page_c']);
   
	//
     //$objSheet->setCellValue('F41',$_POST['sho_cost_c']);
	 $val0 = intval(str_replace(',','',$_POST['sho_cost_c']));
	 $objSheet->setCellValue('F41', $val0);
	 $objSheet->getStyle('F41')->getNumberFormat()->setFormatCode('"\"#,##0');
	 //$objSheet->setCellValue('H41',$_POST['sanno_cost_c']);
	 $val0 = intval(str_replace(',','',$_POST['sanno_cost_c']));
	 $objSheet->setCellValue('H41', $val0);
	 $objSheet->getStyle('H41')->getNumberFormat()->setFormatCode('"\"#,##0');
	 //$objSheet->setCellValue('F42',$_POST['etc_cost_c']);
	 $val0 = intval(str_replace(',','',$_POST['etc_cost_c']));
	 $objSheet->setCellValue('F42', $val0);
	 $objSheet->getStyle('F42')->getNumberFormat()->setFormatCode('"\"#,##0');
